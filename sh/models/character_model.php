...
class Character_model extends MY_Model
{
	...
	function get_character_list($user_id, $chara_id = null){
		$select = "`id`, `character_id` as `CharacterId`, `exp` as `Exp`, `star` as `Star`, `level` as `Level`, `weapon_type` as `WeaponType`, `move_type` as `MoveType`, `horse` as `Horse`, `clothes` as `Clothes`, `weapon` as `Weapon`";
